...
import type { textProps } from '@/types/canvasTypes'

const TextOptions = ({ tool, colorText, setColorText, setTextSize, textSize, setTool }: textProps) => {
  ...
  return (
    <>
      <div
        className={`${tool === 'text' || tool === 'textList' ? '-top-10' : 'top-0'}
  flex gap-2 items-center text-white h-10 w-[90%]  p-2 absolute -z-10  duration-300 ease-in-out rounded-t-lg left-1/2 -translate-x-1/2 bg-black border-x-[1px] border-t-[1px] border-gray-500`}
      >
        ...
        <div
          ...
          data-size={34}
          className={`${
            textSize === 34 && tool === 'text' ? 'bg-blue-500 scale-110' : 'hover:scale-110'
        ...
        <div
          ...
          data-size={30}
          className={`${
            textSize === 30 && tool === 'text' ? 'bg-blue-500 scale-110' : 'hover:scale-110'
          } duration-150 rounded px-[3px] h-fit cursor-pointer`}
        >
          H2
        </div>
        <div
          ...
          data-size={24}
          className={`${
            textSize === 24 && tool === 'text' ? 'bg-blue-500 scale-110' : 'hover:scale-110'
          } duration-150 rounded px-[3px] h-fit cursor-pointer`}
        >
          H3
        </div>
        <div
          ...
          data-size={20}
          className={`${
            textSize === 20 && tool === 'text' ? 'bg-blue-500 scale-110' : 'hover:scale-110'
          } duration-150 rounded px-[3px] h-fit cursor-pointer`}
        >
          H4
        </div>
        <div
          ...
          data-size={14}
          className={`${
            textSize === 14 && tool === 'text' ? 'bg-blue-500 scale-110' : 'hover:scale-110'
          } duration-150 rounded px-[3px] h-fit cursor-pointer`}
        >
          H5
        </div>
        <div
          ...
          data-size={10}
          className={`${
            textSize === 10 && tool === 'text' ? 'bg-blue-500 scale-110' : 'hover:scale-110'
          } duration-150 rounded px-[3px] h-fit cursor-pointer`}
        >
          H6
        </div>
